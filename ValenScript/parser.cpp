...
std::unique_ptr<ASTNode> Parser::statement() {
    ...
    if (currentToken.type == TokenType::PRINT) {
        ...
        if (currentToken.type == TokenType::LBRACKET) {
            ...
            args.push_back(expr());
            while (isExpressionStart(currentToken.type)) {
                args.push_back(expr());
            }
        } else {
...
std::unique_ptr<ASTNode> Parser::factor() {
    ...
    auto hasFutureGt = [this]() {
        std::vector<Token> lookahead = lexer.peekTokens(256);
        for (const auto& t : lookahead) {
            if (t.type == TokenType::GT) return true;
    ...
    auto applyPostfix = [this, &hasFutureGt](std::unique_ptr<ASTNode> node) {
        while (true) {
            if (currentToken.type == TokenType::LT && hasFutureGt()) {
                eat(TokenType::LT);
